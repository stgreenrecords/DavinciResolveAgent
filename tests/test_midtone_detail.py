...
def test_midtone_detail_one_by_one():
    print("Starting Midtone Detail Slider Test (0.0 -> 10.0)...")
    ...
    # Midtone Detail slider coordinates from coordinates.json
    x, y = 1368, 1465
    target_name = "midtone_detail"
    ...
    initial_ss.save("debug/midtone_test_initial.png")
    
    # 2. Read initial value
    initial_val = read_ui_value(initial_ss, x, y)
    print(f"Initial Midtone Detail Value: {initial_val}")
    
    # 3. Execute movement (+10.0 units)
    action = Action(type="set_slider", target=target_name, delta=10.0, reason="Increase Midtone Detail from 0 to 10")
    
    class DummyCalibration:
        def get_target(self, name):
            if name == "midtone_detail":
                return {"x": 1368, "y": 1465}
            return None
        def to_dict(self): return {}

    print("Executing Midtone Detail adjustment...")
    ...
    final_ss.save("debug/midtone_test_final.png")
    
    # 5. Read final value
    final_val = read_ui_value(final_ss, x, y)
    print(f"Final Midtone Detail Value: {final_val}")
    
    # Verification crop for user
    pill_box = (x - 60, y - 20, x + 60, y + 20)
    final_ss.crop(pill_box).save("debug/midtone_value_final.png")
    ...
    if initial_val == 0.0 and final_val == 10.0:
        print("TEST PASSED: Midtone Detail moved from 0.0 to 10.0")
        return True
    else:
...
if __name__ == "__main__":
    success = test_midtone_detail_one_by_one()
