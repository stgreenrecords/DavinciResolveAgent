...
def test_shadows_one_by_one():
    print("Starting Shadows Slider Test (0.0 -> 10.0)...")
    ...
    # Shadows slider coordinates from coordinates.json
    x, y = 430, 1980
    target_name = "shadows_slider"
    ...
    initial_ss.save("debug/shadows_test_initial.png")
    
    # 2. Read initial value
    initial_val = read_ui_value(initial_ss, x, y)
    print(f"Initial Shadows Value: {initial_val}")
    
    # 3. Execute movement (+10.0 units)
    action = Action(type="set_slider", target=target_name, delta=10.0, reason="Increase Shadows from 0 to 10")
    
    class DummyCalibration:
        def get_target(self, name):
            if name == "shadows_slider":
                return {"x": 430, "y": 1980}
            return None
        def to_dict(self): return {}

    print("Executing Shadows adjustment...")
    ...
    final_ss.save("debug/shadows_test_final.png")
    
    # 5. Read final value
    final_val = read_ui_value(final_ss, x, y)
    print(f"Final Shadows Value: {final_val}")
    
    # Verification crop for user
    pill_box = (x - 60, y - 20, x + 60, y + 20)
    final_ss.crop(pill_box).save("debug/shadows_value_final.png")
    ...
    if initial_val == 0.0 and final_val == 10.0:
        print("TEST PASSED: Shadows moved from 0.0 to 10.0")
        return True
    else:
...
if __name__ == "__main__":
    success = test_shadows_one_by_one()
