...
def test_highlights_one_by_one():
    print("Starting Highlights Slider Test (0.0 -> 10.0)...")
    ...
    # Highlights slider coordinates from coordinates.json
    x, y = 664, 1980
    target_name = "highlights_slider"
    ...
    initial_ss.save("debug/highlights_test_initial.png")

    # 2. Read initial value
    initial_val = read_ui_value(initial_ss, x, y)
    print(f"Initial Highlights Value: {initial_val}")

    # 3. Execute movement (+10.0 units)
    action = Action(type="set_slider", target=target_name, delta=10.0, reason="Increase Highlights from 0 to 10")
    ...
    print("Executing Highlights adjustment...")
    ...
    final_ss.save("debug/highlights_test_final.png")

    # 5. Read final value
    final_val = read_ui_value(final_ss, x, y)
    print(f"Final Highlights Value: {final_val}")

    # Verification crop for user
    pill_box = (x - 60, y - 20, x + 60, y + 20)
    final_ss.crop(pill_box).save("debug/highlights_value_final.png")
    ...
    if initial_val == 0.0 and final_val == 10.0:
        print("TEST PASSED: Highlights moved from 0.0 to 10.0")
        return True
    else:
...
if __name__ == "__main__":
    success = test_highlights_one_by_one()
