...
def main():
    ...
    for name, data in config.get("sliders", {}).items():
        if data.get("x") != "" and data.get("y") != "":
            ...
            logger.info(f"[DEBUG] Slider {name}: min={min_val}, max={max_val}, default={default_val}, test_val={median}")
            targets[name] = {"x": x, "y": y}
            test_actions.append(Action(type="set_slider", target=name, value=median, reason=f"Setting {name} to test value {median}"))
    ...
    for wheel_name, components in config.get("wheels", {}).items():
        for comp_name, data in components.items():
            if data.get("x") != "" and data.get("y") != "":
                ...
                logger.info(f"[DEBUG] Wheel {target_name}: min={min_val}, max={max_val}, default={default_val}, test_val={median}")
                targets[target_name] = {"x": x, "y": y}
                test_actions.append(Action(type="set_slider", target=target_name, value=median, reason=f"Setting {target_name} to test value {median}"))
    ...
    print(f"Executing {len(test_actions)} adjustments to median positions...")
    ...
    if len(executed) < len(test_actions) and executor._stop:
        print("TEST STOPPED: User pressed ESC or Pause.")
        sys.exit(0)
    
    if len(executed) < len(test_actions):
        print("TEST FAILED: Not all actions were executed.")
        sys.exit(1)
    ...
    if result == QtWidgets.QMessageBox.Yes:
        ...
        msg2.setStandardButtons(QtWidgets.QMessageBox.Retry | QtWidgets.QMessageBox.Close)
        msg2.setWindowFlags(QtCore.Qt.WindowStaysOnTopHint)
        result2 = msg2.exec()
        if result2 == QtWidgets.QMessageBox.Retry:
